...
pub mod new {
    ...
    impl<T> Surrounding<(usize, T)> {
        pub fn is_between(&self, frame: usize) -> bool {
            match self {
                ...
                Surrounding::End { duration, start } => {
                    frame >= start.0 && frame < start.0 + duration
                }
    ...
    impl<T> Timeline<T> {
        pub fn with_data(
            data: Vec<(usize, T)>,
            duration: usize,
        ) -> Result<Self, TimelineInsertError> {
            let mut ret = Self {
                data: vec![],
                duration,
            };
                    ...
                    if let Some(target_frame) = to_add {
                        let _ = self.insert_frame(
                            ...
                            match self.surrounding(target_frame) {
                                ...
                                Surrounding::End { start, .. }
                                | Surrounding::Pair { start, .. } => start.1.clone(),
                            },
                        );
                    }
    ...
    mod test {
        ...
        fn index_test() {
            ...
            for (idx, test) in (0..7)
                .zip(vec![Some(0), Some(0), Some(2), Some(2), Some(4), None, None].into_iter())
            {
